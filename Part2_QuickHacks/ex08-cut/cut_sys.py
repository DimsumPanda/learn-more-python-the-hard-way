...
parser.add_argument('-f', '--fields', required=True, help='The list specifies fields, separated in the input by the field delimiter \
...
if __name__ == '__main__':
    ...
    for line in sys.stdin:
        ...
        if '-' in args.fields:                
            ...
            int_field = int(args.fields)
            if len(line_split) >= int_field-1:
                if int_field == len(line_split):
                    print(line_split[int_field-1])
                else:
                    print(line_split[int_field-1], end = " ")
